...
export default function AdminInquiries() {
  ...
  const load = () => api.get('/contact').then(({ data }) => setItems(data)).catch(console.error);
  useEffect(() => { load(); }, []);
  ...
  return (
    <div>
      <h1 className="text-2xl font-display font-bold mb-6">Contact Inquiries</h1>
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item.id} onClick={() => setSelected(item)} className={`border-b cursor-pointer hover:bg-gray-50 ${selected?.id === item.id ? 'bg-primary-50' : ''}`}>
                  ...
                  <td className="p-4 text-gray-500 hidden md:table-cell">{item.email}</td>
                  <td className="p-4"><span className="text-xs px-2 py-1 rounded-full bg-blue-100 text-blue-700">{item.status}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
          {items.length === 0 && <p className="p-8 text-center text-gray-500">No inquiries yet.</p>}
        </div>
